...
export async function GET(context: APIContext) {
  ...
  // Filter for Flatpak apps only
  const flatpakApps = apps.filter(app => app.packageType === 'flatpak');
  
  // Create a flat list of releases with their app context
  const allReleases = flatpakApps.flatMap(app => 
  ...
  return rss({
    title: 'Bluefin Firehose - Flatpak Releases',
    description: 'Latest Flatpak application release updates',
