...
export async function GET(context: APIContext) {
  ...
  // Filter for Bluefin OS releases only
  const osReleases = apps.filter(app => app.packageType === 'os');
  
  // Create a flat list of releases with their app context
  const allReleases = osReleases.flatMap(app => 
  ...
  return rss({
    title: 'Bluefin Firehose - OS Releases',
    description: 'Latest Bluefin OS release updates',
